...
class SOLUTION:
    ...
    def Create_Body(self):
        ...
        # Torso centered at z=1.5 so lower legs clear the ground
        pyrosim.Send_Cube(name="Torso", pos=[0, 0, 1.5], size=[1, 1, 1])

        # Back and front legs hang down from y edges of torso
        pyrosim.Send_Joint(name="Torso_BackLeg", parent="Torso", child="BackLeg",
                           type="revolute", position=[0, -0.5, 1.5], jointAxis="1 0 0")
        ...
        # Left and right legs hang down from x edges of torso
        pyrosim.Send_Joint(name="Torso_LeftLeg", parent="Torso", child="LeftLeg",
                           type="revolute", position=[-0.5, 0, 1.5], jointAxis="0 1 0")
        pyrosim.Send_Cube(name="LeftLeg", pos=[-0.5, 0, -0.5], size=[0.2, 0.2, 1])

        pyrosim.Send_Joint(name="Torso_RightLeg", parent="Torso", child="RightLeg",
                           type="revolute", position=[0.5, 0, 1.5], jointAxis="0 1 0")
        pyrosim.Send_Cube(name="RightLeg", pos=[0.5, 0, -0.5], size=[0.2, 0.2, 1])

        # Lower legs hang from bottom of each upper leg
